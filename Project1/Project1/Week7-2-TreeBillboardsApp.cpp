...
private:
	...
	void MagicBuildThing(std::string name, GeometryGenerator::MeshData thing);
	void LoadTextures();
...
void TreeBillboardsApp::MagicBuildThing(std::string name, GeometryGenerator::MeshData thing)
{
	std::vector<Vertex> vertices(thing.Vertices.size());
	for (size_t i = 0; i < thing.Vertices.size(); ++i)
	{
		vertices[i].Pos = thing.Vertices[i].Position;
		vertices[i].Normal = thing.Vertices[i].Normal;
		vertices[i].TexC = thing.Vertices[i].TexC;
	}
	const UINT vbByteSize = (UINT)vertices.size() * sizeof(Vertex);

	std::vector<std::uint16_t> indices = thing.GetIndices16();
	...
	mGeometries[name + "Geo"] = std::move(geo);
}
...
void TreeBillboardsApp::LoadTextures()
{
	...
	treeArrayTex->Filename = L"../../Textures/tree.dds";
	ThrowIfFailed(DirectX::CreateDDSTextureFromFile12(md3dDevice.Get(),
